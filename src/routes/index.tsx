  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "DoctorCita · Salud en Zacatecas" },
      {
        ...
        content:
          "La forma más simple de encontrar un médico y agendar tu cita en Zacatecas.",
      },
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-4 pb-20 pt-14 sm:px-6 lg:grid-cols-[1.05fr_1fr] lg:gap-16 lg:px-8 lg:pt-24">
        <div className="flex flex-col justify-center">
          ...
          <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">
            Más de 40 especialidades, disponibilidad en tiempo real y opiniones
            verificadas de pacientes en todo el estado de Zacatecas.
          </p>
          ...
          <div className="mt-8 rounded-2xl border border-border/70 bg-card p-3 shadow-elevated sm:p-4">
            <div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
              ...
              <div className="relative">
                ...
                <Select value={municipio} onValueChange={setMunicipio}>
                  <SelectTrigger className="h-12 border-transparent bg-surface pl-10 text-sm shadow-none">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 px-1 pt-1 text-xs text-muted-foreground">
              <span>Populares:</span>
              {["Pediatría", "Ginecología", "Dermatología", "Odontología"].map(
                (esp) => (
                  <button
                    key={esp}
                    onClick={() => {
          ...
          <dl className="mt-10 grid max-w-lg grid-cols-3 gap-6 border-t border-border/60 pt-6">
            {[
              { k: "300+", v: "Médicos verificados" },
              { k: "20", v: "Municipios cubiertos" },
              { k: "4.8★", v: "Calificación promedio" },
            ].map((s) => (
              <div key={s.v}>
                <dt className="text-2xl font-semibold text-secondary">{s.k}</dt>
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md">
            ...
            <div className="absolute -left-4 top-8 hidden max-w-[220px] rounded-2xl border border-border bg-card p-4 shadow-elevated sm:block">
              <div className="flex items-center gap-2 text-xs font-medium text-success">
                <span className="h-2 w-2 animate-pulse rounded-full bg-success" />
                Disponibilidad en vivo
              </div>
              <p className="mt-2 text-sm font-medium text-secondary">
                Próxima cita hoy · 16:30
              </p>
              <p className="mt-0.5 text-xs text-muted-foreground">
                Cardiología · Zacatecas Centro
              </p>
            </div>
            ...
            <div className="absolute -bottom-6 -right-4 hidden max-w-[240px] rounded-2xl border border-border bg-card p-4 shadow-elevated sm:block">
              ...
              <p className="mt-2 text-xs font-medium text-secondary">
                "Trato humano, atención puntual y explicaciones claras."
              </p>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      ...
      <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
        {ESPECIALIDADES_DESTACADAS.map((e) => (
          <button
            key={e.nombre}
            onClick={() =>
              navigate({ to: "/medicos", search: { especialidad: e.nombre } })
            }
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {DOCTORS.filter((d) => d.especialidad === e.nombre).length}{" "}
                especialistas
              </div>
            </div>
          </button>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {ESPECIALIDADES.filter(
          (e) => !ESPECIALIDADES_DESTACADAS.some((d) => d.nombre === e),
        )
          .slice(0, 14)
          .map((e) => (
            <button
              key={e}
              onClick={() =>
                navigate({ to: "/medicos", search: { especialidad: e } })
              }
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-primary">{d.especialidad}</div>
                <div className="mt-1 inline-flex items-center gap-1 text-xs text-muted-foreground">
                  <Star className="h-3 w-3 fill-amber-400 text-amber-400" />
                  {d.rating} · {d.reseñas} opiniones
                </div>
              </div>
            </div>
        ...
        <div className="relative grid gap-10 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-white/70 sm:text-base">
              Únete a la plataforma médica de referencia del estado. Nuevos
              pacientes, agenda automatizada y herramientas clínicas modernas
              en un solo lugar.
            </p>
